个人翻译的 [HomeSpan 2.1.8](https://github.com/HomeSpan/HomeSpan)（家庭互联框架），请前往原文 [下载](https://github.com/HomeSpan/HomeSpan/releases) 体验。
...
## ❗最新更新 - HomeSpan 2.1.8（2026 年4 月 26 日）
 
 ### 新功能

* **更新了 HomeSpan 状态逻辑和功能，以改进诊断**
  * 新增 *HS_STATUS* 枚举类型 **HS_CONNECTED**
    * 反映 HomeSpan 已连接到 WiFi 或以太网、已与 HomeKit 配对，并且与 HomeKit 保持一个或多个安全连接的完整运行状态
    * 在此状态下，状态 LED 常亮
  * 修改了 *HS_STATUS* 枚举类型 **HS_PAIRED**
    * 反映 HomeSpan 已连接到 WiFi 或以太网、已与 HomeKit 配对，但尚未与 HomeKit 建立任何安全连接的潜在“无响应”状态
    * 在此状态下，状态 LED 反向双闪
  * 新增 `std::pair<HS_STATUS,uint32_t> homeSpan.getStatus()` 方法
    * 返回包含以下内容的 *std::pair*：
      * 当前 HomeSpan 状态（*HS_STATUS* 枚举类型）
      * 自 HomeSpan 首次变为该状态以来的持续时间（以秒为单位）
    * 此线程安全方法允许用户从主 `loop()` 循环中主动轮询 HomeSpan 状态和持续时间，作为使用 `homeSpan.setStatusCallback()`（仅在 HomeSpan 状态更改时调用）的替代方案
  * 新增 `void homeSpan.resetStatusDuration()` 方法
    * 此线程安全方法允许用户将当前 HomeSpan 状态持续时间重置为零
  * 完全重写 HomeSpan 状态文档
    * 演示如何使用上述新方法
    * 提供示例，说明如果设备长时间失去安全的 HomeKit 连接，如何使用 `homeSpan.getStatus()` 重启 HomeSpan
    * 添加表格，以图形方式显示每个 HomeSpan 状态的所有状态 LED 指示灯模式
    * 在 HomeSpan 主 README.md 页面中添加指向此文档的直接链接
  * 有关详细信息，请参阅[HomeSpan 状态和状态 LED](docs/HS_STATUS.md)。

* **Web 日志输出改进**
  * 新增**客户端连接表**（输出类似于 's' CLI 命令）
    * 用于诊断 HomeSpan 是否与 HomeKit 建立了任何活动的安全连接
  * 添加了描述性类名，以便与自定义样式表一起使用
    * *body* - 背景和标题文本（为了向后兼容，可以使用 *bod1*）
    * *infoTable* - 提供设备常规信息的顶部表格（为了向后兼容，可以使用 *tab1*）
    * *clientTable* - 新增的列出所有活动客户端连接的表格（为了向后兼容，可以使用 *tab2*）
    * *logTable* - 列出所有单独的 Web 日志条目的底部表格（为了向后兼容，也可以使用 *tab2*）
  * 有关详细信息，请参阅[消息日志](docs/Logging.md)。
 
* **新增了从程序中以编程方式检索 Base64 编码的配对数据的功能**
  * 新增了以下两个方法：
    * `const char* homeSpan.getPairingInfo(char **buf)`
      * 为 *buf* 分配内存，并填充 HomeSpan 的设备配对数据
      * 返回 *buf*，不再需要时必须使用 `free(buf)` 释放内存
    * `const char* Controller::getPairingInfo(char **buf)`
      * 为 *buf* 分配内存，并填充 Controller 的配对数据
      * 返回 *buf*，不再需要时必须使用 `free(buf)` 释放内存
  * 使用这些方法可以复制克隆设备所需的“P”CLI 命令提供的完全相同的配件和控制器配对数据
  * 有关这些方法和克隆的详细信息，请参阅[API 参考](docs/Reference.md)。[配对数据](docs/Cloning.md)页面提供了完整的示例

* **新增 `boolean homeSpan.usingEthernet()` 方法**
  * 如果正在使用以太网接口，则返回 true；如果正在使用 WiFi，则返回 false
* **新增 `homeSpan.setControlTimes(uint32_t comTime, uint32_t resTime)` 方法**
  * 允许用户自定义控制按钮触发命令模式或恢复出厂设置所需的按住时间
* **将 `setVal()` 的超出范围警告重定向到 WEBLOG 而不是 LOG0，以便这些警告同时显示在串口监视器和 Web 日志中**
* **更新 `CUSTOM_CHAR()` 逻辑，以便根据宏中指定的范围和名称自动设置 HAP 范围和 HAP 描述**
...
- [HomeSpan 状态和状态 LED](docs/HS_STATUS.md) - 如何以编程方式读取 HomeSpan 的状态并解释状态 LED 的闪烁模式
