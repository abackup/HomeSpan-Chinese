...
| 2.0.0 或以后  | v3.0.2 - **v3.1.1***       | *最小 SPIFFS 分区方案* (1.9MB APP) | ESP32, S2, S3, C3, *还有 C6* |

*HomeSpan 已通过 Arduino-ESP32 开发板管理器 3.1.1 版本（基于 IDF 5.3.2 构建）的测试。后续版本可能可以正常工作，但尚未经过测试。注意 HomeSpan 不支持使用 Arduino-ESP32 开发板管理器的 alpha、beta 或预发布候选版本——仅在开发板管理器的生产版本上进行测试。
