...
// 重要提示！！在 Arduino IDE 下编译时，IDE 本身首先需要确保草图的大小不大于分区表中应用程序分区的大小。
// 但是，IDE 无法访问您的自定义“partition.csv”文件，而是根据您在 IDE 菜单中选择的分区方案计算应用程序分区的大小，
// 即使如果您有自己的“partitions.csv”文件，该方案实际上并未使用，如本例所示。在编译之前，您必须在 IDE 中选择一个分区方案，
// 该方案的应用程序分区大小与自定义“partitions.csv”文件中的应用程序分区大小相匹配。
