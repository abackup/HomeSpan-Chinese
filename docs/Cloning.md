...
除了使用 'P' 命令行界面命令在串口监视器上显示设备的配对数据之外，您还可以通过编程方式从程序中获取此信息。对于配件配对数据，可以使用 `homeSpan.getPairingInfo()` 方法；对于每个控制器的配对数据，可以使用控制器链表实现的 `pairingInfo()` 方法。例如，'P”命令行界面命令本身就使用这些方法来获取和显示设备的配对数据，如下所示：

```C++
// 提取并打印与 HomeSpan 在使用 'P' CLI 命令时打印到串口监视器的相同的配对数据

Serial.printf("\n*** Pairing Data used for Cloning another Device\n\n");

char *buf;      // 声明一个字符缓冲区，用于存储配对数据。

Serial.printf("Accessory data:  %s\n", homeSpan.getPairingInfo(&buf) );    // 传递指向缓冲区的指针——该函数将返回填充了设备配对数据的同一缓冲区。
free(buf);                                                                 // 别忘了释放为 getPairingInfo 方法分配的用于存储配对数据的内存。

for(auto it=homeSpan.controllerListBegin(); it!=homeSpan.controllerListEnd(); ++it){    // 创建一个迭代器来遍历所有配对控制器
  Serial.printf("Controller data: %s\n", it->getPairingInfo(&buf));                     // 您可以使用上面声明的同一个缓冲区。
  free(buf);                                                                            // 再次使用缓冲区之前，务必先释放已分配的空间。
}
