...
通过将 *nSeconds* 设置为足够长的时间以允许所有设置过程运行（可能为 10-20 秒），您可以确保如果任何 HomeSpan 过程陷入无限循环，设备将自行重启。这在与 OTA 回滚结合使用时特别有用，因为您可以配置一些东西，这样如果您通过 OTA 上传新示例并且它在启动时挂起，它将在短时间后重启并回滚到示例的先前工作版本。有关如何启用回滚的详细信息，请参阅 [HomeSpan OTA](OTA.md) 页面。
