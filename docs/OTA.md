...
您也可以通过调用 `homeSpan.enableOTA(const char *pwd)` 在草图中以编程方式更改密码，其中 *pwd* 是纯文本密码，HomeSpan 会在将其转换为哈希值后再将其保存到 NVS 中。但是，这不如使用 "O" 命令安全，因为这意味着您的草图中包含了纯文本密码的副本。

或者，您可以使用 `homeSpan.enableOTA(const char *pwd)` 来提供您自己预先计算的纯文本密码的哈希值，而不是指定实际的纯文本密码，如下所示：

* 如果 *pwd* 以 "0x" 开头，后跟*正好* 64 位十六进制数字，HomeSpan 会将 *pwd* 解释为 SHA256 哈希值
* 如果 *pwd* 以 "0x" 开头，后跟*正好* 32 个十六进制数字，HomeSpan 会将 *pwd* 解释为 MD5 哈希值

当 HomeSpan 将 *pwd* 解释为哈希值时，它会将其直接保存在 NVS 中。上述逻辑也适用于使用 "O" 命令设置密码 - 如果您指定的密码是 SH256 或 MD5 哈希值的形式，它将被解释为 SH256 或 MD5 哈希值并直接保存在 NVS 中。由于 SHA256 比 MD5 更安全，因此 HomeSpan 在创建纯文本密码的哈希值时始终使用 SHA256。如果您设置了自定义哈希值，建议使用 SHA256。如果您指定了 MD5 哈希值，则每当您通过 OTA 上传草图时，Arduino IDE 都会警告您使用了安全性较低的哈希值。

请注意，使用 `homeSpan.enableOTA(const char *pwd)` 在草图中设置纯文本密码（或纯文本密码的哈希值）会导致 HomeSpan 使用该 *pwd* 而不是使用 "O" 命令在 NVS 中保存的哈希值，但它不会改变已保存的密码。
