...
准备好开始创建自己的 HomeSpan 示例了吗？ 查看 [服务和特性](ServiceList.md) 以获取 HomeSpan 支持的所有 HomeKit 服务和特性的完整列表，以及 [附件类别](Categories.md) 以获取所有支持的 HomeKit 类别列表。并且不要忘记使用 [Apple (HAP-R2)](../master/docs/HAP-R2.pdf) 作为你对每项服务和详细信息的参考特性。
