...
* 在某些 ESP32 开发板上，USB-UART 芯片只有在通过 USB 端口供电时才会获得电源，因此如果 ESP32 仅通过 5V 引脚供电，则该芯片将处于未通电状态。这样一来，通常由 USB-UART 芯片驱动的 ESP32 上与 UART0 关联的串口接收引脚 (Serial RX) 就会处于任意浮空状态。如果该引脚处于低电平，ESP32 会误以为串口监视器有数据需要读取，导致 HomeSpan 程序陷入无限循环，试图读取这些不存在的数据。**发生这种情况时，您的程序将无法正常响应，因为主循环 `loop()` 并未运行！**
