...
*  `int fade(float level, uint32_t fadeTime, int fadeType=LedPin::ABSOLUTE)`
   ...
   * 这是一个 **NON-BLOCKING** 方法，将立即返回。渐变效果发生在由 ESP32 硬件控制的背景中
   * 注意：一旦渐变效果开始，就不能停止或更改，直到完成（这是 ESP32 硬件的限制）。
   * 如果已成功开始渐变效果，则此方法返回 0；如果渐变效果已在进行中且尚无法更改，则此方法返回 1（在特定 LEDpin 的淡入淡出已在进行中时，新的淡入淡出请求将被忽略）。
   * 使用下面的 *fadeStatus* 方法来确定任何给定 LedPin 的当前渐变状态
...
*  `int fadeStatus()`
   ...
   * 返回 LEDPIN 的淡化状态。返回值如下：
     ...
     * **LedPin::FADING** - LEDPIN 上的渐变效果当前正在进行，无法更改/停止
     * **LedPin::COMPLETED** - 渐变效果刚刚完成
       * 返回此值后，对的 `fadeStatus()` 后续调用将返回 **LedPin::NOT_FADING**（除非你再次调用 `fade()`）
       * 通过在 `loop()` 方法中检查 `fadeStatus()==LedPin::COMPLETED`，你可以在渐变效果完成后触发新的操作（如果需要）
